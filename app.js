...
var curTab='home', nutrDate='', logDate='', exPart='Grudi', stab='trening', nperiod='week';
var charts={};
...
function widx(date){var d=new Date(date+'T12:00:00');return(d.getDay()+6)%7}
...
window.rHome=function(){
  ...
  var li=widx(logDate),ls=S.schedule[li];
  var ldone=S.workouts.filter(function(w){return w.date===logDate});
  var ldt=new Date(logDate+'T12:00:00');
  var llbl=ldt.toLocaleDateString('sr-Latn',{weekday:'long',day:'numeric',month:'long'});
  ...
  h+='<div class="card">';
  h+='<div class="dnav"><button onclick="chLD(-1)">◀</button>';
  h+='<div style="text-align:center"><div style="font-size:14px;font-weight:600">'+llbl+'</div>';
  if(logDate===today())h+='<div style="font-size:11px;color:var(--grn)">Danas</div>';
  h+='</div><button onclick="chLD(1)">▶</button></div>';
  if(ls.active){
    h+='<div style="font-size:16px;font-weight:600;color:var(--accL);margin-bottom:10px">🕐 '+ls.time+'</div>';
    h+='<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px">';
    ls.parts.forEach(function(p){h+='<span class="bdg bb">'+p+'</span>'});
    h+='</div>';
    if(!ldone.length){
      h+='<button class="btn btp" onclick="openLog('+li+')">+ Zapiši trening</button>';
    }else{
      h+='<div class="bdg bg" style="font-size:14px;padding:8px 14px;border-radius:10px">✓ Trening upisan</div>';
    }
  }else{
    h+='<div class="empty">'+(logDate===today()?'Danas':'Ovog dana')+' nije dan treninga 💤</div>';
    if(!ldone.length)h+='<button class="btn bts" style="margin-top:10px" onclick="openLog('+li+')">+ Ipak zapiši trening</button>';
  }
  h+='</div>';
  if(last){
...
function chLD(d){var dt=new Date(logDate+'T12:00:00');dt.setDate(dt.getDate()+d);logDate=dt.toISOString().split('T')[0];rHome();}

// ===== LOG MODAL =====
function openLog(dayIdx){
  var sc=S.schedule[dayIdx];
  var dt=logDate||today();
  S.log={di:dayIdx,date:dt,parts:sc.parts,note:'',exercises:S.exercises.filter(function(e){return sc.parts.indexOf(e.part)>=0}).map(function(e){
...
function rLog(){
  ...
  if(L.date&&L.date!==today()){
    var ldt=new Date(L.date+'T12:00:00'),llbl=ldt.toLocaleDateString('sr-Latn',{weekday:'long',day:'numeric',month:'long'});
    h+='<div style="font-size:12px;color:var(--amb);margin-bottom:12px;margin-top:-6px">📅 Unos za: '+llbl+'</div>';
  }
  h+='<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px">';
...
function saveLog(){
  if(!S.log)return;
  S.workouts.push({id:'w'+Date.now(),date:S.log.date||today(),day:S.log.di,parts:S.log.parts,note:S.log.note,exercises:JSON.parse(JSON.stringify(S.log.exercises))});
